...
void ResultsWindow::setResults(std::vector<Team> &loadedTeams)
{
	...
	for (int i = 0; i < loadedTeams.size(); i++)
	{
		for (int j = 0; j < loadedTeams[i].getKeys().size(); j++)
		{
			qDebug() << "\nTeam: " << loadedTeams[i].getTeamName()
					 << "\nSouvenir: " << loadedTeams[i].getSouvenirs().Find(loadedTeams[i].getKeys()[j]).Value().getName()
					 << "\nPrice: " << loadedTeams[i].getSouvenirs().Find(loadedTeams[i].getKeys()[j]).Value().getPrice()
					 << "\Quantity: " << loadedTeams[i].getSouvenirs().Find(loadedTeams[i].getKeys()[j]).Value().getQuantity()
					 << '\n';
		}
	}
	...
	for (int i = 0; i < loadedTeams.size(); i++)
	{
		for (int j = 0; j < loadedTeams[i].getKeys().size(); j++)
		{
			if (loadedTeams[i].getSouvenirs().Find(loadedTeams[i].getKeys()[j]).Value().getQuantity() > 0)
			{
				numSouvenirs = numSouvenirs + loadedTeams[i].getSouvenirs().Find(loadedTeams[i].getKeys()[j]).Value().getQuantity();
				if (pushback)
				{
	...
	for (int i = 0; i < loadedTeams.size(); i++)
	{
		...
		for (int j = 0; j < loadedTeams[i].getKeys().size(); j++)
		{
			if (loadedTeams[i].getSouvenirs().Find(loadedTeams[i].getKeys()[j]).Value().getQuantity() > 0)
			{
				...
				label = new QLabel();
				label->setText(loadedTeams[i].getSouvenirs().Find(loadedTeams[i].getKeys()[j]).Value().getName());
				ui->souvenirTable->setCellWidget(souvenirsLoaded, 1, label);

				// set the price for an individual food item
				label = new QLabel();
				label->setText("$" + QString::number(loadedTeams[i].getSouvenirs().Find(loadedTeams[i].getKeys()[j]).Value().getPrice()));
				ui->souvenirTable->setCellWidget(souvenirsLoaded, 2, label);

				// set the quantity purchases
				label = new QLabel();
				label->setText("x" + QString::number(loadedTeams[i].getSouvenirs().Find(loadedTeams[i].getKeys()[j]).Value().getQuantity()));
				ui->souvenirTable->setCellWidget(souvenirsLoaded, 3, label);

				// display total Price for the food items
				label = new QLabel();
				label->setText("$" + QString::number(loadedTeams[i].getSouvenirs().Find(loadedTeams[i].getKeys()[j]).Value().calculateTotal()));
				ui->souvenirTable->setCellWidget(souvenirsLoaded, 4, label);

				grandTotal = grandTotal + loadedTeams[i].getSouvenirs().Find(loadedTeams[i].getKeys()[j]).Value().calculateTotal();
				souvenirTotal = souvenirTotal + loadedTeams[i].getSouvenirs().Find(loadedTeams[i].getKeys()[j]).Value().calculateTotal();
				souvenirsLoaded++;
			}
		}
		if (pushback)
